...
from entityspine.domain.graph import Relationship, Event, NodeRef, NodeKind
...
class IngestResult:
    ...
    registrant_entity_id: Optional[str] = None
...
def ingest_filing_facts(store: "SqliteStore", facts: FilingFacts) -> IngestResult:
    ...
    if facts.registrant_ticker:
        security, listing = _create_security_and_listing(
            registrant_entity, facts
        )
    ...
    for extracted_rel in facts.relationships:
        ...
        if source_id and target_id:
            ...
            result.warnings.append(
                f"Could not link relationship: missing {', '.join(missing)}"
            )
...
def _create_security_and_listing(
    entity: Entity, facts: FilingFacts
) -> tuple[Security, Listing]:
...
def _create_entity_from_extraction(
    extracted: ExtractedEntity, facts: FilingFacts
) -> Entity:
    """Create Entity from extracted entity mention."""
    entity_type = _ENTITY_TYPE_MAP.get(
        extracted.entity_type.lower(), EntityType.UNKNOWN
    )
...
def _create_event_from_extraction(
    ...
    event_type = _EVENT_TYPE_MAP.get(
        extracted.event_type.lower(), EventType.OTHER
    )
