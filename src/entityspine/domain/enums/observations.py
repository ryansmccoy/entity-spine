...
class MetricCode(str, Enum):
    ...
    DEBT_TO_EQUITY = "debt_to_equity"  # Alias
...
class AccountingBasis(str, Enum):
    ...
    STATUTORY = "statutory"  # Local statutory reporting
    REGULATORY = "regulatory"  # Regulatory basis (banks, insurance)
    OTHER = "other"
...
class Presentation(str, Enum):
    ...
    REPORTED = "reported"  # As filed (GAAP/IFRS)
    COMPANY_ADJUSTED = "company_adjusted"  # Company non-GAAP
    VENDOR_NORMALIZED = "vendor_normalized"  # Vendor adjustment
    PRO_FORMA = "pro_forma"  # Pro forma presentation
...
class PeriodType(str, Enum):
    ...
    LTM = "ltm"  # Last twelve months (synonym for TTM)
    NTM = "ntm"  # Next twelve months (forward)
    MONTHLY = "monthly"
    WEEKLY = "weekly"
    DAILY = "daily"
...
class EstimateScope(str, Enum):
    ...
    BROKER: Individual analyst/firm estimate
    CONSENSUS: Aggregated consensus
    COMPANY_GUIDANCE: Company-provided guidance
    """

    BROKER = "broker"
    CONSENSUS = "consensus"
    COMPANY_GUIDANCE = "company_guidance"
    BUY_SIDE = "buy_side"  # Buy-side internal estimate
